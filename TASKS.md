...
- [x] T-0029 `wongo roundtrip` located the scaffold example's inserted "efficiently" on a line inside the writing-rules HTML comment; `locate()` now skips comment lines and anchors on the context's last words (`da6ff85`; found while making the lab-meeting deck).
