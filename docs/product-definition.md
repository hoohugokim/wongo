...
- **Install:** `uv tool install --editable .` (dev) or `uv tool install dist/wongo-0.1.0-py3-none-any.whl` / `pip install -e .` from the GitHub Release. PyPI publication is deferred (name `wongo` reserved, not yet published) — see `CHANGELOG.md:33` and `HANDOFF-wongo-uplift.md:63`.
