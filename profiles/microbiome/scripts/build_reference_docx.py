...
# ///
"""Build assets/reference.docx for quarto-manuscript-microbiome.

Reproducible: pandoc's default reference doc restyled per Microbiome (BMC)
...
and read Microbiome's own Submission guidelines page in full: it specifies
double-line spacing, line and page numbering, no page breaks, and accepted
file formats (DOC/DOCX/RTF/TeX-LaTeX) — but no body font family or point
size is stated anywhere in the fetched guidance. In the absence of a
verified body-text font requirement, this uses the same standard,
portal-safe serif default as quarto-manuscript-est's, quarto-manuscript-
npjcw's, and quarto-manuscript-natwater's reference docs — TNR 12pt.
Revisit if a future verification pass finds an explicit Microbiome
manuscript-body font/size requirement.
"""
import subprocess
import tempfile
