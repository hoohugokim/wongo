...
- **Analysis** — "a new analysis of existing data, describes new data
  ...
  **Best-fit format for a comparative benchmark/evaluation-style manuscript**
  (original evaluation data across models/tasks): its own definition explicitly
  names "a new analysis of existing data... a comparative analysis... a new
  simulation or model" — re-derive the fit privately per manuscript.
- **Perspective** — "a review of a topic from a personal viewpoint... more
  ...
  only, not OA-eligible. **Important constraint**: the explicit "may not
  contain primary research data" rule means a benchmark-style paper reporting
  ORIGINAL evaluation results does not fit Perspective's own stated
  constraints — see `references/editorial-framing.md` for why Analysis, not
  Perspective, is this journal's correct format for such work.
...
Given Nature Water's engineering/CS-adjacent scope, a computational/
benchmark-for-water-technology manuscript likely falls OUTSIDE the life-sciences/
...
**AI policy** (relevant to any AI-subject-matter manuscript — verbatim from
...
*benchmarks* LLMs is not itself constrained by this policy beyond normal
disclosure of any LLM used to help write the paper text.
...
- [ ] Whether an AI-benchmark-for-water-technology Analysis manuscript
...
See `references/editorial-framing.md` for journal facts (content-type rules,
selectivity context: JIF 30.7, no external editorial board, 141-day median
time to acceptance). The manuscript-specific fit argument was removed before
this repository was published (it named an unpublished submission); re-derive
it privately when targeting this journal.
